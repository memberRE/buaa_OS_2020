                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	//printf("____load_icode_mapper in begin\n");
    ...
    * hint: variable `i` has the value of `bin_size` now! */
	...
	//printf("___load_icode_mapper enddddd\n");
	return 0;
}
/* Overview:
...
load_icode(struct Env *e, u_char *binary, u_int size)
{
	...
	//printf("in load_icode :page insert end\n");
    if (load_elf(binary, size, &entry_point, (void*)e, load_icode_mapper) < 0) {
        return;
    }
	//printf("in load_icode:load_elf endd\n");
...
env_create_priority(u_char *binary, int size, int priority)
{
	...
	//printf("env_create____middle\n");
    load_icode(e, binary, size);
	//printf("env_create___load_icode_end\n");
    LIST_INSERT_HEAD(&env_sched_list[0],e,env_sched_link);
	//printf("env_create______enddd\n");
}
/* Overview:
